...
namespace EXILED.Events.Patches
{
	...
	public class HandcuffEvent
	{
		public static bool Prefix(Handcuffs __instance, GameObject target)
		{
			...
			if (handcuffs == null || __instance.MyReferenceHub.inventory.curItem != ItemType.Disarmer || (__instance.MyReferenceHub.characterClassManager.CurClass < RoleType.Scp173 || handcuffs.CufferId >= 0) || handcuffs.MyReferenceHub.inventory.curItem != ItemType.None)
				return false;
			Team team1 = __instance.MyReferenceHub.characterClassManager.Classes.SafeGet(__instance.MyReferenceHub.characterClassManager.CurClass).team;
			Team team2 = __instance.MyReferenceHub.characterClassManager.Classes.SafeGet(handcuffs.MyReferenceHub.characterClassManager.CurClass).team;
			bool flag = false;
			switch (team1)
			{
				case Team.MTF:
			...
			bool allow = true;
			Events.Events.InvokePlayerHandcuff(__instance.gameObject, target, ref allow);
	...
	public class UncuffTeammate
	{
		public static bool Prefix(Handcuffs __instance, GameObject target)
		{
			...
			if (!__instance._interactRateLimit.CanExecute(true) || target == null || (Vector3.Distance(target.transform.position, __instance.transform.position) > __instance.raycastDistance * 1.10000002384186 || __instance.MyReferenceHub.characterClassManager.Classes.SafeGet(__instance.MyReferenceHub.characterClassManager.CurClass).team == Team.SCP))
				return false;
			bool allow = true;
			Events.Events.InvokePlayerHandcuffFree(__instance.gameObject, target, ref allow);
			if (!allow)
				return false;
	...
	public class ClearTargets
	{
		public static bool Prefix(Handcuffs __instance)
		{
			...
			foreach (GameObject player in PlayerManager.players)
			{
				...
				{
					bool allow = true;
					Events.Events.InvokePlayerHandcuffFree(__instance.gameObject, player, ref allow);
					if (allow)
