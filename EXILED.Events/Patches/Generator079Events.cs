			...
			try
			{
				if (command.StartsWith("EPS_TABLET"))
				{
					...
					foreach (Inventory.SyncItemInfo syncItemInfo in component.items)
					{
						...
						{
							bool allow = true;
							Events.Events.InvokeGeneratorInsert(person, __instance, ref allow);
							if (!allow)
								return false;
				...
				else if (command.StartsWith("EPS_CANCEL"))
				{
					if (!__instance.isTabletConnected)
						return false;
					bool allow = true;
					Events.Events.InvokeGeneratorEject(person, __instance, ref allow);
					if (allow)
				...
				else if (command.StartsWith("EPS_DOOR"))
				{
					...
					if (!__instance.isDoorUnlocked)
					{
						...
						Events.Events.InvokeGeneratorUnlock(person, __instance, ref allow);
					...
					{
						bool allow = true;
						Events.Events.InvokeGeneratorOpen(person, __instance, ref allow);
						if (!allow)
						{
							__instance.RpcDenied();
							return false;
	...
	public class Generator079Finish
	{
		public static bool Prefix(Generator079 __instance)
		{
			if (__instance.prevFinish || __instance.localTime <= 0.0)
				Events.Events.InvokeGeneratorFinish(__instance);
			return true;
		}
	}
}