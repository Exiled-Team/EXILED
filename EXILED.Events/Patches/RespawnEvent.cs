    ...
    public class RespawnEvent
    {
        public static bool Prefix(MTFRespawn __instance)
        {
            ...
            try
            {
                ...
                LogHelper.Debug($"Respawn: Got players: {players.Count}");
                foreach (GameObject player in players.ToArray())
                    if (player.GetComponent<ServerRoles>().OverwatchEnabled)
                    {
                        LogHelper.Debug($"Removing {ReferenceHub.GetHub(player)} -- Overwatch true");
                ...
                LogHelper.Debug($"Respawn: pre-vent list: {toRespawn.Count}");
                Events.Events.InvokeTeamRespawn(ref isChaos, ref maxRespawn, ref toRespawn);
                ...
                foreach (GameObject ply in toRespawn)
                {
                    if (!(ply == null))
                    {
                        ...
                        if (isChaos)
                        {
                            ...
                            __instance.playersToNTF.Add((GameObject)ply);
                    }
                }
            ...
            {
                LogHelper.Error($"Respawn Event error: {e}");
                return true;
            }
        }
    }
