    ...
    public abstract class CustomRole
    {
        ...
        private void OnInternalDying(DyingEventArgs ev)
        {
            if (Check(ev.Player))
            {
                ...
                Ragdoll.CreateAndSpawn(new RagdollData(ev.Player.ReferenceHub, ev.DamageHandler, Role, ev.Player.Position, ev.Player.Rotation, ev.Player.DisplayNickname, NetworkTime.time));
            }
        }
    }
}
