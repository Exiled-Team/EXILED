...
using Sexiled.Loader.Features.Configs;
    ...
    public static class ConfigManager
    {
        ...
        public static SortedDictionary<string, IConfig> LoadSorted(string rawConfigs)
        {
            try
            {
                ...
                if (!rawDeserializedConfigs.TryGetValue("sexiled_loader", out object rawDeserializedConfig))
                {
                    Log.Warn($"Sexiled.Loader doesn't have default configs, generating...");

                    deserializedConfigs.Add("sexiled_loader", Loader.Config);
                }
                else
                {
                    deserializedConfigs.Add("sexiled_loader", Deserializer.Deserialize<Config>(Serializer.Serialize(rawDeserializedConfig)));

                    Loader.Config.CopyProperties(deserializedConfigs["sexiled_loader"]);
                }
