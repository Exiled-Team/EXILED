...
namespace Exiled.Events.Features
{
    ...
    public class Event<T> : IExiledEvent
    {
        ...
        /// <summary>
        ///     Gets a <see cref="IReadOnlyCollection{T}" /> of <see cref="Event{T}" /> which contains all the
        ///     <see cref="Event{T}" /> instances.
        /// </summary>
        public static IReadOnlyDictionary<Type, Event<T>> Dictionary => TypeToEvent;

        /// <summary>
        ///     Subscribes a target <see cref="CustomEventHandler{TEventArgs}" /> to the inner event and checks if patching is
        ///     possible, if dynamic patching is enabled.
        /// </summary>
        ...
        /// <summary>
        ///     Subscribes a <see cref="CustomAsyncEventHandler" /> to the inner event, and checks patches if dynamic patching is
        ///     enabled.
        /// </summary>
        /// <param name="event">The <see cref="Event{T}" /> to subscribe the <see cref="CustomAsyncEventHandler{T}" /> to.</param>
        /// <param name="asyncEventHandler">
        ///     The <see cref="CustomAsyncEventHandler{T}" /> to subscribe to the
        ///     <see cref="Event{T}" />.
        /// </param>
        ...
        /// <summary>
        ///     Unsubscribes a target <see cref="CustomEventHandler{TEventArgs}" /> from the inner event and checks if unpatching
        ///     is possible, if dynamic patching is enabled.
        /// </summary>
        /// <param name="event">The <see cref="Event{T}" /> the <see cref="CustomEventHandler{T}" /> will be unsubscribed from.</param>
        /// <param name="handler">
        ///     The <see cref="CustomEventHandler{T}" /> that will be unsubscribed from the
        ///     <see cref="Event{T}" />.
        /// </param>
        ...
        /// <summary>
        ///     Unsubscribes a target <see cref="CustomAsyncEventHandler{TEventArgs}" /> from the inner event, and checks if
        ///     unpatching is possible, if dynamic patching is enabled.
        /// </summary>
        /// <param name="event">The <see cref="Event" /> the <see cref="CustomAsyncEventHandler{T}" /> will be unsubscribed from.</param>
        /// <param name="asyncEventHandler">
        ///     The <see cref="CustomAsyncEventHandler{T}" /> that will be unsubscribed from the
        ///     <see cref="Event{T}" />.
        /// </param>
        /// <returns>The <see cref="Event{T}" /> with the handler unsubscribed from it.</returns>
        public static Event<T> operator -(Event<T> @event, CustomAsyncEventHandler<T> asyncEventHandler)
        ...
        /// </summary>
        /// <param name="handler">The handler to add.</param>
        public void Unsubscribe(CustomEventHandler<T> handler) { InnerEvent -= handler; }

        /// <summary>
        ///     Unsubscribes a target <see cref="CustomEventHandler{T}" /> from the inner event if the conditional is true.
        /// </summary>
        /// <param name="handler">The handler to add.</param>
        public void Unsubscribe(CustomAsyncEventHandler<T> handler) { InnerAsyncEvent -= handler; }
