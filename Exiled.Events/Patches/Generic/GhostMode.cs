                    ...
                    else if (player.Role != RoleType.Spectator && player.Role != RoleType.Scp079)
                    {
                        for (int index = 0; index < __instance._usedData; ++index)
                        {
                            ...
                            else
                            {
                                ...
                                switch (player.Role)
                                {
                                    case RoleType.Scp173 when (!Exiled.Events.Events.Instance.Config.CanTutorialBlockScp173 && currentTarget.Role == RoleType.Tutorial) || Scp173.TurnedPlayers.Contains(currentTarget.Id):
                                        shouldRotate = true;
                                        break;
