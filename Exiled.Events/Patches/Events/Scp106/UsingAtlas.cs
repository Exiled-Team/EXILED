...
namespace Exiled.Events.Patches.Events.Scp106
{
    ...
    ///     Adds the <see cref="Handlers.Scp106.UsingAtlas" /> event.
    /// </summary>
    ...
    internal static class UsingAtlas
    {
        private static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            LocalBuilder ev = generator.DeclareLocal(typeof(UsingHunterAtlasEventArgs));
            ...
            newInstructions.InsertRange(
                ...
                new CodeInstruction[]
                {
                    ...
                    new(OpCodes.Newobj, GetDeclaredConstructors(typeof(UsingHunterAtlasEventArgs))[0]),
                    new(OpCodes.Dup),
                    new(OpCodes.Dup),
                    new(OpCodes.Stloc_S, ev.LocalIndex),

                    // Scp106.OnTeleporting(ev)
                    new(OpCodes.Call, Method(typeof(Scp106), nameof(Scp106.OnUsingAtlas))),

                    // if (!ev.IsAllowed)
                    //    return;
                    new(OpCodes.Callvirt, PropertyGetter(typeof(UsingHunterAtlasEventArgs), nameof(UsingHunterAtlasEventArgs.IsAllowed))),
                    new(OpCodes.Brfalse_S, returnLabel),

                    // position = ev.Position
                    new(OpCodes.Ldloc_S, ev.LocalIndex),
                    new(OpCodes.Callvirt, PropertyGetter(typeof(UsingHunterAtlasEventArgs), nameof(UsingHunterAtlasEventArgs.Position))),
                    new(OpCodes.Stloc_0),
                });
