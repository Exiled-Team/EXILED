                         ...
                         else
                         {
                           ...
                           try
                           {
                             if (!ECDSA.VerifyBytes(
                               $"{(object)result6};{(object)result8};{(object)result9};{(object)result7}", result10, ServerConsole.PublicKey))
                             {
                               ServerConsole.AddLog(
                             ...
                             else
                             {
                               ...
                               if (!flags.HasFlagFast(CentralAuthPreauthFlags.IgnoreBans) || !ServerStatic.GetPermissionsHandler().IsVerified)
                               {
                                 ...
                                 {
                                   ServerConsole.AddLog(
                                     $"{(keyValuePair.Key == null ? (object)"Player" : (object)"Banned player")} {(object)result6} tried to connect from {(keyValuePair.Value == null ? (object)string.Empty : (object)"banned ")} endpoint {(object)request.RemoteEndPoint}.");
                                   ServerLogs.AddLog(ServerLogs.Modules.Networking, $"{(keyValuePair.Key == null ? (object)"Player" : (object)"Banned player")} {(object)result6} tried to connect from {(keyValuePair.Value == null ? (object)string.Empty : (object)"banned ")} endpoint {(object)request.RemoteEndPoint}.", ServerLogs.ServerLogType.ConnectionUpdate);
                                   CustomLiteNetLib4MirrorTransport.RequestWriter.Reset();
                                   CustomLiteNetLib4MirrorTransport.RequestWriter.Put((byte)6);
