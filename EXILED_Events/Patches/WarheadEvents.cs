	...
	public class WarheadPlayerStartEvent
	{
		public static bool Prefix(PlayerInteract __instance)
		{
			...
			try
			{
				...
				if ((AlphaWarheadController._resumeScenario == -1 && Math.Abs(AlphaWarheadController.Host.scenarios_start[AlphaWarheadController._startScenario].SumTime() - AlphaWarheadController.Host.timeToDetonation) < 0.0001f) ||
					(AlphaWarheadController._resumeScenario != -1 && Math.Abs(AlphaWarheadController.Host.scenarios_resume[AlphaWarheadController._resumeScenario].SumTime() - AlphaWarheadController.Host.timeToDetonation) < 0.0001f))
				{
					bool allow = true;
