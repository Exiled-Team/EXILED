...
namespace EXILED.Patches
{
	...
	public class PickupItemEvent
	{
		...
		{
			if (!__instance._playerInteractRateLimit.CanExecute(true) || t == null || (!__instance.hub.characterClassManager.IsHuman() || Vector3.Distance(__instance.GetComponent<PlyMovementSync>().RealModelPosition, t.transform.position) > 3.5))
        return false;
      Pickup component = t.GetComponent<Pickup>();
      if (component == null || !__instance._pickupInProgressServer || (t != __instance._pickupObjectServer || __instance._pickupProgressServer > 0.25) || component.info.locked)
        return false;
      ...
      if (itemById1.noEquipable)
      {
        for (int type = 0; type < __instance.hub.ammoBox.types.Length; ++type)
        {
          if (__instance.hub.ammoBox.types[type].inventoryID == component.info.itemId)
          {
            ...
            int num = __instance.hub.characterClassManager.Classes.SafeGet(__instance.hub.characterClassManager.CurClass).maxAmmo[type];
            int durability;
      ...
      else
      {
        ...
        switch (itemCategory)
        {
          ...
          case ItemCategory.NoCategory:
            ...
            if (allow == false)
              return false;
            __instance.AddItem(itemId, (Object) t.GetComponent<Pickup>() == (Object) null ? -1f : component.info.durability, component.info.weaponMods);
            break;
          default:
            int num = 0;
