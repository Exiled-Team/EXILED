            ...
            if (!CharacterClassManager.OnlineMode)
            {
              ...
              else
              {
                ...
                ServerLogs.AddLog(ServerLogs.Modules.Networking,
                  $"{result3} preauthenticated from endpoint {request.RemoteEndPoint}.", ServerLogs.ServerLogType.ConnectionUpdate);
              }
            }
            else
            {
                    ...
                    else
                    {
                      ...
                      else
                      {
                        ...
                        if (LiteNetLib4MirrorCore.Host.ConnectedPeersCount < num)
                        {
                          ...
                          bool allow = true;
                          Events.InvokePreAuth(result6, request, position, result8, result9, ref allow);
                          if (allow)
                          {
                            request.Accept();
                            CustomLiteNetLib4MirrorTransport.PreauthDisableIdleMode();
                          ...
                          else
                          {
                            ...
                            ServerLogs.AddLog(ServerLogs.Modules.Networking,
                              $"{result3} tried to preauthenticate from endpoint {request.RemoteEndPoint}, but the request has been rejected by a plugin.", ServerLogs.ServerLogType.ConnectionUpdate);
                          }
                        }
                        else
                        {
