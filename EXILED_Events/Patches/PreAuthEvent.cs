            ...
            else
            {
              ...
              else
              {
                ...
                try
                {
                  ...
                  {
                    ServerConsole.AddLog(string.Format("Player from endpoint {0} sent preauthentication token with invalid digital signature.", (object) request.RemoteEndPoint), ConsoleColor.Gray);
                    CustomLiteNetLib4MirrorTransport.RequestWriter.Reset();
                  ...
                  {
                    ServerConsole.AddLog(string.Format("Player from endpoint {0} sent expired preauthentication token.", (object) request.RemoteEndPoint), ConsoleColor.Gray);
                  ...
                  else
                  {
                    if (CustomLiteNetLib4MirrorTransport.UserRateLimiting)
                    {
                      ...
                      {
                        ServerConsole.AddLog(string.Format("Incoming connection from {0} ({1}) rejected due to exceeding the rate limit.", (object) result6, (object) request.RemoteEndPoint), ConsoleColor.Gray);
                        ServerLogs.AddLog(ServerLogs.Modules.Networking, string.Format("Incoming connection from endpoint {0} ({1}) rejected due to exceeding the rate limit.", (object) result6, (object) request.RemoteEndPoint), ServerLogs.ServerLogType.RateLimit);
                        CustomLiteNetLib4MirrorTransport.RequestWriter.Reset();
                    ...
                    if (!flags.HasFlagFast(CentralAuthPreauthFlags.IgnoreBans) || !ServerStatic.GetPermissionsHandler().IsVerified)
                    {
                      ...
                      {
                        ServerConsole.AddLog(string.Format("{0} {1} tried to connect from {2} endpoint {3}.", keyValuePair.Key == null ? (object) "Player" : (object) "Banned player", (object) result6, keyValuePair.Value == null ? (object) "" : (object) "banned ", (object) request.RemoteEndPoint), ConsoleColor.Gray);
                        ServerLogs.AddLog(ServerLogs.Modules.Networking, string.Format("{0} {1} tried to connect from {2} endpoint {3}.", keyValuePair.Key == null ? (object) "Player" : (object) "Banned player", (object) result6, keyValuePair.Value == null ? (object) "" : (object) "banned ", (object) request.RemoteEndPoint), ServerLogs.ServerLogType.ConnectionUpdate);
                        CustomLiteNetLib4MirrorTransport.RequestWriter.Reset();
                        CustomLiteNetLib4MirrorTransport.RequestWriter.Put((byte) 6);
                      ...
                      if (LiteNetLib4MirrorCore.Host.ConnectedPeersCount < num)
                      {
                        ...
                        if (allow)
                        {
                          ...
                          ServerLogs.AddLog(ServerLogs.Modules.Networking,
                            $"{result6} preauthenticated from endpoint {request.RemoteEndPoint}.", ServerLogs.ServerLogType.ConnectionUpdate);
                        }
                        else
                        {
                          ServerConsole.AddLog(
                            $"Player {result6} tried to preauthenticate from endpoint {request.RemoteEndPoint}, but the request has been rejected by a plugin.");
                          ServerLogs.AddLog(ServerLogs.Modules.Networking,
                            $"{result6} tried to preauthenticate from endpoint {request.RemoteEndPoint}, but the request has been rejected by a plugin.", ServerLogs.ServerLogType.ConnectionUpdate);
                        }
                        ServerConsole.AddLog(string.Format("Player {0} preauthenticated from endpoint {1}.", (object) result6, (object) request.RemoteEndPoint), ConsoleColor.Gray);
                        ServerLogs.AddLog(ServerLogs.Modules.Networking, string.Format("{0} preauthenticated from endpoint {1}.", (object) result6, (object) request.RemoteEndPoint), ServerLogs.ServerLogType.ConnectionUpdate);
                        CustomLiteNetLib4MirrorTransport.PreauthDisableIdleMode();
                      }
                      else
                      {
