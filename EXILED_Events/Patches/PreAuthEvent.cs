...
namespace EXILED.Patches
{
  [HarmonyPatch(typeof(CustomLiteNetLib4MirrorTransport),
    nameof(CustomLiteNetLib4MirrorTransport.ProcessConnectionRequest), typeof(ConnectionRequest))]
    ...
    private static void HandleConnection(ConnectionRequest request)
    {
      try
      {
        ...
        if (!request.Data.TryGetByte(out result1) || !request.Data.TryGetByte(out result2) ||
            (!request.Data.TryGetByte(out result3) || (int) result1 != (int) CustomNetworkManager.Major) ||
            ((int) result2 != (int) CustomNetworkManager.Minor ||
             (int) result3 != (int) CustomNetworkManager.SyncedRevision))
        {
        ...
        else
        {
          ...
          else if (CustomLiteNetLib4MirrorTransport.DelayConnections)
          {
            ...
            if ((int) CustomLiteNetLib4MirrorTransport.DelayVolume <
                (int) CustomLiteNetLib4MirrorTransport.DelayVolumeThreshold)
            {
              ServerConsole.AddLog(
                string.Format("Delayed connection incoming from endpoint {0} by {1} seconds.",
                  (object) request.RemoteEndPoint, (object) CustomLiteNetLib4MirrorTransport.DelayTime),
                ConsoleColor.Gray);
            ...
            {
              ServerConsole.AddLog(
                string.Format("Force delayed connection incoming from endpoint {0} by {1} seconds.",
                  (object) request.RemoteEndPoint, (object) CustomLiteNetLib4MirrorTransport.DelayTime),
                ConsoleColor.Gray);
              request.RejectForce(CustomLiteNetLib4MirrorTransport.RequestWriter);
            }
          ...
          else
          {
            if (CustomLiteNetLib4MirrorTransport.UseChallenge)
            {
              if (result4 == 0 || result5 == null || result5.Length == 0)
              {
                ...
                for (byte index = 0; index < (byte) 3; ++index)
                {
                  ...
                  if (CustomLiteNetLib4MirrorTransport.Challenges.ContainsKey(key))
                  {
                    if (index == (byte) 2)
                    {
                      ...
                      request.RejectForce(CustomLiteNetLib4MirrorTransport.RequestWriter);
                      ServerConsole.AddLog(
                        string.Format("Failed to generate ID for challenge for incoming connection from endpoint {0}.",
                ...
                byte[] bytes = RandomGenerator.GetBytes(
                  (int) CustomLiteNetLib4MirrorTransport.ChallengeInitLen +
                  (int) CustomLiteNetLib4MirrorTransport.ChallengeSecretLen, true);
                ServerConsole.AddLog(
                  string.Format("Requested challenge for incoming connection from endpoint {0}.",
                    (object) request.RemoteEndPoint), ConsoleColor.Gray);
                CustomLiteNetLib4MirrorTransport.RequestWriter.Reset();
                CustomLiteNetLib4MirrorTransport.RequestWriter.Put((byte) 13);
                CustomLiteNetLib4MirrorTransport.RequestWriter.Put(
                  (byte) CustomLiteNetLib4MirrorTransport.ChallengeMode);
                ...
                switch (CustomLiteNetLib4MirrorTransport.ChallengeMode)
                {
                  case ChallengeType.MD5:
                    CustomLiteNetLib4MirrorTransport.RequestWriter.PutBytesWithLength(bytes, 0,
                      (int) CustomLiteNetLib4MirrorTransport.ChallengeInitLen);
                    ...
                    CustomLiteNetLib4MirrorTransport.Challenges.Add(key,
                      new PreauthChallengeItem(new ArraySegment<byte>(bytes,
                        (int) CustomLiteNetLib4MirrorTransport.ChallengeInitLen,
                        (int) CustomLiteNetLib4MirrorTransport.ChallengeSecretLen)));
                    break;
              ...
              if (!CustomLiteNetLib4MirrorTransport.Challenges.ContainsKey(key1))
              {
                ServerConsole.AddLog(
                  string.Format(
                    "Security challenge response of incoming connection from endpoint {0} has been REJECTED (invalid Challenge ID).",
                    (object) request.RemoteEndPoint), ConsoleColor.Gray);
                CustomLiteNetLib4MirrorTransport.RequestWriter.Reset();
              ...
              if (!((IEnumerable<byte>) result5).SequenceEqual<byte>((IEnumerable<byte>) validResponse))
              {
                ServerConsole.AddLog(
                  string.Format(
                    "Security challenge response of incoming connection from endpoint {0} has been REJECTED (invalid response).",
                    (object) request.RemoteEndPoint), ConsoleColor.Gray);
                CustomLiteNetLib4MirrorTransport.RequestWriter.Reset();
                CustomLiteNetLib4MirrorTransport.RequestWriter.Put((byte) 15);
                request.RejectForce(CustomLiteNetLib4MirrorTransport.RequestWriter);
              ...
              CustomLiteNetLib4MirrorTransport.PreauthDisableIdleMode();
              ServerConsole.AddLog(
                string.Format("Security challenge response of incoming connection from endpoint {0} has been accepted.",
                  (object) request.RemoteEndPoint), ConsoleColor.Gray);
            }
            ...
            if (!CharacterClassManager.OnlineMode)
            {
              KeyValuePair<BanDetails, BanDetails> keyValuePair =
                BanHandler.QueryBan((string) null, request.RemoteEndPoint.Address.ToString());
              if (keyValuePair.Value != null)
              {
                ServerConsole.AddLog(
                  string.Format("Player tried to connect from banned endpoint {0}.", (object) request.RemoteEndPoint),
                  ConsoleColor.Gray);
                CustomLiteNetLib4MirrorTransport.RequestWriter.Reset();
            ...
            else
            {
              ...
              else
              {
                ...
                string result9;
                byte[] result10;
                if (!request.Data.TryGetULong(out result7) || !request.Data.TryGetByte(out result8) ||
                    (!request.Data.TryGetString(out result9) || !request.Data.TryGetBytesWithLength(out result10)))
                {
                  CustomLiteNetLib4MirrorTransport.RequestWriter.Reset();
                  CustomLiteNetLib4MirrorTransport.RequestWriter.Put((byte) 4);
                  request.RejectForce(CustomLiteNetLib4MirrorTransport.RequestWriter);
                ...
                else
                {
                  ...
                  try
                  {
                    if (!ECDSA.VerifyBytes(
                      string.Format("{0};{1};{2};{3}", (object) result6, (object) result8, (object) result9,
                        (object) result7), result10, ServerConsole.PublicKey))
                    {
                      ServerConsole.AddLog(
                        string.Format(
                          "Player from endpoint {0} sent preauthentication token with invalid digital signature.",
                          (object) request.RemoteEndPoint), ConsoleColor.Gray);
                      CustomLiteNetLib4MirrorTransport.RequestWriter.Reset();
                    ...
                    {
                      ServerConsole.AddLog(
                        string.Format("Player from endpoint {0} sent expired preauthentication token.",
                          (object) request.RemoteEndPoint), ConsoleColor.Gray);
                      ServerConsole.AddLog(
                    ...
                    else
                    {
                      if (CustomLiteNetLib4MirrorTransport.UserRateLimiting)
                      {
                        if (CustomLiteNetLib4MirrorTransport.UserRateLimit.Contains(result6))
                        {
                          ServerConsole.AddLog(
                            string.Format(
                              "Incoming connection from {0} ({1}) rejected due to exceeding the rate limit.",
                              (object) result6, (object) request.RemoteEndPoint), ConsoleColor.Gray);
                          ServerLogs.AddLog(ServerLogs.Modules.Networking,
                            string.Format(
                              "Incoming connection from endpoint {0} ({1}) rejected due to exceeding the rate limit.",
                              (object) result6, (object) request.RemoteEndPoint), ServerLogs.ServerLogType.RateLimit);
                          CustomLiteNetLib4MirrorTransport.RequestWriter.Reset();
                      ...
                      {
                        KeyValuePair<BanDetails, BanDetails> keyValuePair =
                          BanHandler.QueryBan(result6, request.RemoteEndPoint.Address.ToString());
                        if (keyValuePair.Key != null || keyValuePair.Value != null)
                        {
                          ServerConsole.AddLog(
                            string.Format("{0} {1} tried to connect from {2} endpoint {3}.",
                              keyValuePair.Key == null ? (object) "Player" : (object) "Banned player", (object) result6,
                              keyValuePair.Value == null ? (object) "" : (object) "banned ",
                              (object) request.RemoteEndPoint), ConsoleColor.Gray);
                          ServerLogs.AddLog(ServerLogs.Modules.Networking,
                            string.Format("{0} {1} tried to connect from {2} endpoint {3}.",
                              keyValuePair.Key == null ? (object) "Player" : (object) "Banned player", (object) result6,
                          ...
                          CustomLiteNetLib4MirrorTransport.RequestWriter.Put(
                            keyValuePair.Key?.Reason ?? keyValuePair.Value?.Reason ?? string.Empty);
                          request.Reject(CustomLiteNetLib4MirrorTransport.RequestWriter);
                          return;
                        }
                      }
                      ...
                      if (flags.HasFlagFast(CentralAuthPreauthFlags.GloballyBanned) &&
                          (ServerStatic.PermissionsHandler.IsVerified ||
                           CustomLiteNetLib4MirrorTransport.UseGlobalBans))
                      {
                        ServerConsole.AddLog(
                          string.Format("Player {0} ({1}) kicked due to an active global ban.", (object) result6,
                            (object) request.RemoteEndPoint), ConsoleColor.Gray);
                        CustomLiteNetLib4MirrorTransport.RequestWriter.Reset();
                        CustomLiteNetLib4MirrorTransport.RequestWriter.Put((byte) 8);
                        request.Reject(CustomLiteNetLib4MirrorTransport.RequestWriter);
                      }
                      else if ((!flags.HasFlagFast(CentralAuthPreauthFlags.IgnoreWhitelist) ||
                                !ServerStatic.GetPermissionsHandler().IsVerified) && !WhiteList.IsWhitelisted(result6))
                      {
                        ServerConsole.AddLog(
                          string.Format("Player {0} tried joined from endpoint {1}, but is not whitelisted.",
                            (object) result6, (object) request.RemoteEndPoint), ConsoleColor.Gray);
                        CustomLiteNetLib4MirrorTransport.RequestWriter.Reset();
                        ...
                        request.Reject(CustomLiteNetLib4MirrorTransport.RequestWriter);
                               ...
                               (!CustomLiteNetLib4MirrorTransport.GeoblockIgnoreWhitelisted ||
                                !WhiteList.IsOnWhitelist(result6)) &&
                               (CustomLiteNetLib4MirrorTransport.Geoblocking == GeoblockingMode.Whitelist &&
                                !CustomLiteNetLib4MirrorTransport.GeoblockingList.Contains(result9.ToUpper()) ||
                                CustomLiteNetLib4MirrorTransport.Geoblocking == GeoblockingMode.Blacklist &&
                                CustomLiteNetLib4MirrorTransport.GeoblockingList.Contains(result9.ToUpper())))
                      {
                        ServerConsole.AddLog(
                          string.Format("Player {0} ({1}) tried joined from blocked country {2}.", (object) result6,
                            (object) request.RemoteEndPoint, (object) result9.ToUpper()), ConsoleColor.Gray);
                        CustomLiteNetLib4MirrorTransport.RequestWriter.Reset();
                      ...
                      else
                      {
                        ...
                        if (flags.HasFlagFast(CentralAuthPreauthFlags.ReservedSlot) &&
                          ...
                          num = LiteNetLib4MirrorNetworkManager.singleton.maxConnections;
                                 ...
                                 ReservedSlot.HasReservedSlot(result6))
                        ...
                        if (LiteNetLib4MirrorCore.Host.ConnectedPeersCount < num)
                        {
                          if (CustomLiteNetLib4MirrorTransport.UserIds.ContainsKey(request.RemoteEndPoint))
                            ...
                            CustomLiteNetLib4MirrorTransport.UserIds.Add(request.RemoteEndPoint,
                              new PreauthItem(result6));
                          bool allow = true;
                          Events.InvokePreAuth(result6, request, request.Data.Position, result8, result9, ref allow);
                          if (allow)
                          {
                            request.Accept();
                            CustomLiteNetLib4MirrorTransport.PreauthDisableIdleMode();
                            ServerConsole.AddLog(
                              $"Player {result6} preauthenticated from endpoint {request.RemoteEndPoint}.");
                            ServerLogs.AddLog(ServerLogs.Modules.Networking,
                              $"{result6} preauthenticated from endpoint {request.RemoteEndPoint}.",
                  ...
                  {
                    ServerConsole.AddLog(
                      string.Format("Player from endpoint {0} sent an invalid preauthentication token. {1}",
                        (object) request.RemoteEndPoint, (object) ex.Message), ConsoleColor.Gray);
                    CustomLiteNetLib4MirrorTransport.RequestWriter.Reset();
                    CustomLiteNetLib4MirrorTransport.RequestWriter.Put((byte) 2);
                    request.RejectForce(CustomLiteNetLib4MirrorTransport.RequestWriter);
                  }
      ...
      {
        ServerConsole.AddLog(
          string.Format("Player from endpoint {0} failed to preauthenticate: {1}", (object) request.RemoteEndPoint,
            (object) ex.Message), ConsoleColor.Gray);
        CustomLiteNetLib4MirrorTransport.RequestWriter.Reset();
        CustomLiteNetLib4MirrorTransport.RequestWriter.Put((byte) 4);
        request.RejectForce(CustomLiteNetLib4MirrorTransport.RequestWriter);
      }
