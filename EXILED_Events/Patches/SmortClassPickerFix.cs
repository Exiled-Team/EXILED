			...
			try
			{
				...
				foreach (GameObject gameObject3 in shuffledPlayerList)
				{
					if (!(gameObject3 == null))
					{
						...
						if (component2 != null)
						{
							networkConnection2 = (component2.connectionToClient ?? component2.connectionToServer);
						}
				...
				foreach (GameObject gameObject4 in list2)
				{
					...
					else
					{
						...
						ServerLogs.AddLog(ServerLogs.Modules.ClassChange,
							...
							") class set to SPECTATOR by Smart Class Picker.", ServerLogs.ServerLogType.GameEvent);
					}
				}
				...
				{
					ServerConsole.AddLog(rtr.Key.GetComponent<NicknameSync>().MyNick + " role set to " + rtr.Value);
