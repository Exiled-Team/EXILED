...
namespace Exiled.CustomModules.API.Commands.CustomItem.List
{
    ...
    internal sealed class Tracked : ICommand
    {
        ...
        public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
        {
            ...
            {
                if (!customItem.Items.Any())
                    continue;

                message.AppendLine()
                    .Append('[').Append(customItem.Id).Append(". ").Append(customItem.Name).Append(" (").Append(customItem.ItemType).Append(')')
                    .Append(" {").Append(customItem.Items.Count()).AppendLine("}]").AppendLine();

                count += customItem.Items.Count();

                foreach (Item insideInventory in customItem.Items)
                {
                    Player owner = Player.List.FirstOrDefault(player => player.Inventory.UserInventory.Items.Any(item => item.Key == insideInventory.Serial));
