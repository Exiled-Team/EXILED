...
#pragma warning disable SA1313
    ...
    internal static class SendingRemoteAdminCommand
    {
        private static bool Prefix(ref string q, ref CommandSender sender)
        {
            ...
            var ev = new SendingRemoteAdminCommandEventArgs(sender, string.IsNullOrEmpty(sender.SenderId) ? API.Features.Server.Host : API.Features.Player.Get(sender.SenderId) ?? API.Features.Server.Host, name, arguments.ToList());
            ...
            if (q.StartsWith("gban-kick", System.StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    sender.RaReply(
                        $"GBAN-KICK# Permission to run command denied by the server. If this is an unexpected error, contact SEXILED developers.",
                        false,
                        true,
                        string.Empty);

                    Log.Error($"A user {sender.Nickname} attempted to run GBAN-KICK and was denied permission. If this is an unexpected error, contact SEXILED developers.");
