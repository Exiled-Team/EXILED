...
#pragma warning disable SA1123 // Do not place regions within elements
    ...
    internal static class ExplodingFragGrenade
    {
        private static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            var players = generator.DeclareLocal(typeof(Dictionary<API.Features.Player, float>));

            // Declare Dictionary<Player, float>.Enumerator local variable.
            var playerEnumerator = generator.DeclareLocal(typeof(Dictionary<API.Features.Player, float>.Enumerator));

            // Declare KeyValuePair<Player, float> local variable.
            var playerKeyValuePair = generator.DeclareLocal(typeof(KeyValuePair<API.Features.Player, float>));
            ...
            {
                new CodeInstruction(OpCodes.Newobj, GetDeclaredConstructors(typeof(Dictionary<API.Features.Player, float>))[0]),
            ...
            newInstructions.InsertRange(firstIndex, new[]
            {
                ...
                new CodeInstruction(OpCodes.Callvirt, Method(typeof(Dictionary<API.Features.Player, float>), nameof(Dictionary<API.Features.Player, float>.Add), new[] { typeof(API.Features.Player), typeof(float) })),
            });
            ...
            {
                new CodeInstruction(OpCodes.Ldloc_S, players.LocalIndex),
                new CodeInstruction(OpCodes.Callvirt, Method(typeof(Dictionary<API.Features.Player, float>), nameof(Dictionary<API.Features.Player, float>.GetEnumerator))),
                ...
                new CodeInstruction(OpCodes.Call, PropertyGetter(typeof(Dictionary<API.Features.Player, float>.Enumerator), nameof(Dictionary<API.Features.Player, float>.Enumerator.Current))),
            ...
            var foreachBody = new[]
            {
                ...
                new CodeInstruction(OpCodes.Dup),
                new CodeInstruction(OpCodes.Call, PropertyGetter(typeof(KeyValuePair<API.Features.Player, float>), nameof(KeyValuePair<API.Features.Player, float>.Value))),
                new CodeInstruction(OpCodes.Stloc_S, 14),
                new CodeInstruction(OpCodes.Call, PropertyGetter(typeof(KeyValuePair<API.Features.Player, float>), nameof(KeyValuePair<API.Features.Player, float>.Key))),
            ...
            var foreachEnd = new[]
            {
                ...
                new CodeInstruction(OpCodes.Call, Method(typeof(Dictionary<API.Features.Player, float>.Enumerator), nameof(Dictionary<API.Features.Player, float>.Enumerator.MoveNext))),
                ...
                new CodeInstruction(OpCodes.Constrained, typeof(Dictionary<API.Features.Player, float>.Enumerator)),
