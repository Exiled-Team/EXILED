...
#pragma warning disable SA1313
    ...
    internal static class ExplodingFlashGrenade
    {
        private static bool Prefix(FlashGrenade __instance)
        {
            try
            {
                Dictionary<API.Features.Player, float> players = new Dictionary<API.Features.Player, float>();
                ...
                foreach (GameObject gameObject in PlayerManager.players)
                {
                    ...
                    {
                        players.Add(API.Features.Player.Get(gameObject), num);
                    }
                }

                ExplodingGrenadeEventArgs ev = new ExplodingGrenadeEventArgs(API.Features.Player.Get(__instance.throwerGameObject), players, false, __instance.gameObject);
