    ...
    internal static class Hurting
    {
        private static bool Prefix(PlayerStats __instance, ref PlayerStats.HitInfo info, GameObject go)
        {
            try
            {
                ...
                {
                    Handlers.Scp079.OnRecontained(new RecontainedEventArgs(target));
                    var eventArgs = new DiedEventArgs(null, target, info);
                    Handlers.Player.OnDied(eventArgs);
                }
                ...
                if (!ev.Target.IsGodModeEnabled && (ev.Amount == -1 || ev.Amount >= ev.Target.Health + ev.Target.ArtificialHealth))
                {
                    ...
                    Handlers.Player.OnDying(dyingEventArgs);
            ...
            {
                Log.Error($"Sexiled.Events.Patches.Events.Player.Hurting: {e}\n{e.StackTrace}");
                return true;
            }
        }
    }
