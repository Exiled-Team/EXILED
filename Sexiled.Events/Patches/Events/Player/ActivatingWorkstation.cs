        ...
        private static bool Prefix(WorkStation __instance, GameObject tabletOwner)
        {
            try
            {
                ...
                foreach (Inventory.SyncItemInfo syncItemInfo in component.items)
                {
                    ...
                    {
                        ActivatingWorkstationEventArgs ev = new ActivatingWorkstationEventArgs(API.Features.Player.Get(tabletOwner), __instance);
            ...
            {
                API.Features.Log.Error($"Sexiled.Events.Patches.Events.Player.ActivatingWorkstation: {exception}\n{exception.StackTrace}");
                return true;
            }
        }
    }
