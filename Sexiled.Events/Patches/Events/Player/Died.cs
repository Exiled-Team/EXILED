...
#pragma warning disable SA1118
    ...
    internal static class Died
    {
        private static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            var attacker = generator.DeclareLocal(typeof(API.Features.Player));

            // Declare the target local variable.
            var target = generator.DeclareLocal(typeof(API.Features.Player));
            ...
            newInstructions.InsertRange(index, new[]
            {
                ...
                new CodeInstruction(OpCodes.Callvirt, PropertyGetter(typeof(Component), nameof(Component.gameObject))),
                new CodeInstruction(OpCodes.Call, Method(typeof(API.Features.Player), nameof(API.Features.Player.Get), new[] { typeof(GameObject) })).WithLabels(attacterPlayerGetAfterInstance),
                ...
                new CodeInstruction(OpCodes.Callvirt, PropertyGetter(typeof(API.Features.Player), nameof(API.Features.Player.IsGodModeEnabled))),
