...
namespace Sexiled.Events.Handlers.Internal
{
    ...
    /// <summary>
    /// Handles <see cref="Sexiled.Events.Handlers.Map.Generated"/> event.
    /// </summary>
    internal static class MapGenerated
    {
        ...
        private static void GenerateRooms()
        {
            ...
            if (pocket == null)
                Log.Send($"[{typeof(API.Features.Map).FullName}]: Pocket Dimension not found. The name or location in the game's hierarchy might have changed.", Discord.LogLevel.Error, ConsoleColor.DarkRed);
            else
            ...
            if (surface == null)
                Log.Send($"[{typeof(API.Features.Map).FullName}]: Surface not found. The name in the game's hierarchy might have changed.", Discord.LogLevel.Error, ConsoleColor.DarkRed);
            else
                roomObjects.Add(surface);

            foreach (var roomObject in roomObjects)
                API.Features.Map.RoomsValue.Add(Room.CreateComponent(roomObject));

            ListPool<GameObject>.Shared.Return(roomObjects);
        }

        private static void GenerateDoors() => API.Features.Map.DoorsValue.AddRange(UnityEngine.Object.FindObjectsOfType<DoorVariant>());

        private static void GenerateCameras() => API.Features.Map.CamerasValue.AddRange(UnityEngine.Object.FindObjectsOfType<Camera079>());

        private static void GenerateLifts() => API.Features.Map.LiftsValue.AddRange(UnityEngine.Object.FindObjectsOfType<Lift>());

        private static void GenerateTeslaGates() => API.Features.Map.TeslasValue.AddRange(UnityEngine.Object.FindObjectsOfType<TeslaGate>());
    }
}
