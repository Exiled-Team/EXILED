// -----------------------------------------------------------------------
// <copyright file="WeaponAttachmentDesyncFix.cs" company="SEXiled Team">
    ...
    internal static class WeaponAttachmentDesyncFix
    {
        private static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            newInstructions.AddRange(new[]
            {
                ...
                new CodeInstruction(OpCodes.Ldfld, Field(typeof(InventorySystem.Items.Firearms.Firearm), nameof(InventorySystem.Items.Firearms.Firearm.CombinedAttachments))),
                new CodeInstruction(OpCodes.Ldfld, Field(typeof(AttachmentSettings), nameof(AttachmentSettings.AdditionalPros))),
                new CodeInstruction(OpCodes.Ldc_I4_2),
                new CodeInstruction(OpCodes.Call, Method(typeof(AttachmentsUtils), nameof(AttachmentsUtils.HasFlagFast), new System.Type[] { typeof(AttachmentDescriptiveAdvantages), typeof(AttachmentDescriptiveAdvantages) })),
                ...
                new CodeInstruction(OpCodes.Br_S, skipLabel2),
                new CodeInstruction(OpCodes.Ldc_I4_S, 12).WithLabels(skipLabel),
                new CodeInstruction(OpCodes.Ldloc_2).WithLabels(skipLabel2),
                new CodeInstruction(OpCodes.Newobj, GetDeclaredConstructors(typeof(InventorySystem.Items.Firearms.FirearmStatus))[0]),
                new CodeInstruction(OpCodes.Call, PropertySetter(typeof(InventorySystem.Items.Firearms.Firearm), nameof(InventorySystem.Items.Firearms.Firearm.Status))),
